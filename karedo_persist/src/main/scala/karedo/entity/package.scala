...
import karedo.entity.dao.Keyable

/**
  * Created by charaj on 20/01/2017.
  */
package object entity extends DefaultJsonProtocol
  ...
  /*
  implicit object KeyableJsonFormat extends RootJsonFormat[Keyable[_]] {
    def write(obj: Keyable[_]) = obj match {
      case f: RootJsonFormat[Keyable[_]] => f.toJson
      case unknown @ _ => serializationError(s"Marshalling issue with ${unknown}")
    }

    def read(obj: JsValue) = obj match {
      case k: Keyable[_] => obj.convertTo[Keyable[_]]
