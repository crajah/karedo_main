...
/**
 * pakkio: taken from sources of play 2.3.4 adapted to not require logging which was
 * difficult to provide (at least for me) from the build.sbt script
...
import _root_.sbt._
import sbt.testing.{Event => TEvent, NestedSuiteSelector, NestedTestSelector, Status => TStatus, TestSelector}
 ...
 */
class JenkinsJUnitXmlTestsListener(val outputDir: String)
  ...
  val logger = Logger(LoggerFactory.getLogger("JenkinsJUnitXmlTestsListener"))
