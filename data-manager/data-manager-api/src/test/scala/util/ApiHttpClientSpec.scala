...
trait ApiHttpClientSpec extends SpecificationLike with NoTimeConversions with Mockito {
  ...
  trait WithMockedPersistenceRestService extends After {
    ...
    val server = new RestServiceWithMockPersistence(servicePort, mockedBrandDAO, mockedAdvDAO,
      mockedClientApplicationDAO, mockedUserAccountDAO, mockedMediaDAO, mockedOfferDAO, messagerActor.ref)
