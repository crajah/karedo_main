...
object BrandActor {
  ...
  implicit object RegistrationErrorJsonFormat extends RootJsonFormat[BrandError] {
        ...
        case JsObject(attributes) => {
          attributes.get("type") match {
            case Some(JsString("BrandInvalidRequest")) =>
              val data = attributes.get("data") map {
                _.asInstanceOf[JsObject]
              }
              data match {
                case None => InvalidBrandRequest("Unknown reason")
                case Some(errorData) =>
                  val reason = errorData.fields.get("reason") map {
                    _.toString
                  } getOrElse ("Unknown")
                  InvalidBrandRequest(reason)
              }
          }
        }
      }
