...
case class AdvertisementDetail(@Key("_id") id: UUID = UUID.randomUUID(), text: String, imagePaths: List[String], value: Int)
case class Brand(@Key("_id") id: UUID = UUID.randomUUID(), name: String = "", iconPath: String = "", ads: List[AdvertisementMetadata]=List())